...
//					   " "+arg1+","+arg2+","+arg3);
	...
	protected static UUID toUUID(char[] data, int offset) {
		...
		// TeaVM does not emulate the UUID(long, long) constructor, but it does
		// emulate UUID.fromString. Build the canonical 8-4-4-4-12 hex string
		// from the two longs and round-trip through fromString so this class
		// stays reachable on the web backend. (This file is a web-only copy of
		// ANTLR 4.7's ATNDeserializer — the desktop jar still uses the
		// upstream version.)
		String hi = padHex16(mostSigBits);
		String lo = padHex16(leastSigBits);
